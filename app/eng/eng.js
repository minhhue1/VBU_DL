...
function getNewQuestion() {
    ...
    question.innerText = currentQuestion.vietnamese;
...
function openPopup() {
    ...
    expText = currentQuestion.vietnamese + ": " + currentQuestion.english;
...
function checkAnswer() {
    ...
    const correctAnswers = generateCombinations(currentQuestion.english);
    console.log(correctAnswers);

    // Lưu câu hỏi và câu trả lời của người dùng
    userAnswers.push({
        question: currentQuestion.vietnamese,
...
/**
 * Hàm tạo tổ hợp từ các chuỗi có dấu []
 */
function generateCombinations(input) {
    ...
    if (!match) {
        // Nếu không còn nhóm [] nào, trả về chuỗi gốc
        return [input.trim()];
    }

    const options = match[1].split('/').map(option => option.trim()); // Các lựa chọn trong []
    const before = input.slice(0, match.index).trim(); // Phần trước []
    const after = input.slice(match.index + match[0].length).trim(); // Phần sau []

    // Đệ quy để tạo tổ hợp
    const combinations = [];
    for (const option of options) {
        const subCombinations = generateCombinations(`${before} ${option} ${after}`.trim());
